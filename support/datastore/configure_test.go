...
func TestConfigureDatastore(t *testing.T) {
	...
	t.Run("creates new manifest", func(t *testing.T) {
		...
		manifest, ok, err := ConfigureDatastore(ctx, mockDataStore, defaultCfg)
		require.NoError(t, err)
		require.True(t, ok)
	...
	t.Run("returns error if PutFile fails", func(t *testing.T) {
		...
		_, ok, err := ConfigureDatastore(ctx, mockDataStore, defaultCfg)
		require.Error(t, err)
		require.False(t, ok)
...
func TestLoadSchema(t *testing.T) {
	...
	// Manifest file not found (backward compatibility)
	t.Run("Manifest not found", func(t *testing.T) {
		...
		schema, err := LoadSchema(ctx, mockOS, defaultCfg)
		require.Error(t, err)
		require.Contains(t, err.Error(), "failed to validate datastore manifest: file does not exist")
	...
	t.Run("Manifest found but validation fails", func(t *testing.T) {
		...
		schema, err := LoadSchema(ctx, mockOS, defaultCfg)
		require.Error(t, err)
		require.Contains(t, err.Error(), "datastore configuration mismatch")
		require.EqualValues(t, DataStoreSchema{}, schema)
		mockOS.AssertExpectations(t)
	})
}
